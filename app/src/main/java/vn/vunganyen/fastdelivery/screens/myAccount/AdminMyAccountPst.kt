...
class AdminMyAccountPst {
    ...
    fun validCheck(curentPw: String, pw1: String, pw2: String) {
        ...
        if (curentPw.isEmpty() || pw1.isEmpty() || pw2.isEmpty()) {
            adminMyAccountItf.ErrorIsEmpty()
            return
        }
        if (SplashActivity.PASSWORD.matcher(pw1).matches() && SplashActivity.PASSWORD.matcher(pw2).matches())
        {
            ...
            } else adminMyAccountItf.ErrorConfirmPw()
        } else adminMyAccountItf.RegexPassword()
    }

    fun checkAccount(curentPw: String, pw1: String){
        var req = ChangePwReq(tendangnhap,curentPw)
        println("kaka")
        ApiAuthService.Api.api.checkAuth(SplashActivity.token,req).enqueue(object : Callback<CheckWordRes>{
    ...
    fun changePassword(newPassword : String){
        ...
        println("haha")
        ApiAuthService.Api.api.changePassword(SplashActivity.token,req).enqueue(object : Callback<UpdateRes>{
            override fun onResponse(call: Call<UpdateRes>, response: Response<UpdateRes>) {
                if(response.isSuccessful){
                    println("hihi")
                    adminMyAccountItf.ChangePwSuccess()
                }
                else adminMyAccountItf.ChangePwFail()
            }
